...
/** Container for making application context components configurable and accessible.
 *
 * Lifecycle: (instantiate)--> creating --.initialize()--> initialized --.start()--> started --.stop()-->  stopped
 *
 * The container can only once be started and stopped.
 *
 * Usage:
 * Make your application context inherit from Container.  Eg MyAppContext extends Container
 * Then in the constructor, use the add and addFactory methods to add components and factories to the container and
 * at the end of the constructor, call initialize();
 */
...
public class Container {
  ...
  /** list of unique components, so you can get a list of components in the order as they are added. */
  ...
  /** true if the component is already in the list of objects using == (not .equals()). */
  ...
  /** add a factory to create an object of a given clazz when it is retrieved from the container */
  ...
  public void start() {
    assertTrue(state==State.INITIALIZED, "To initialize, the container should be in state INITIALIZED, but was "+state);
